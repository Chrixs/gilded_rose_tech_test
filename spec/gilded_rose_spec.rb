...
describe GildedRose do
  ...
  describe "#update_quality" do
    ...
    context 'Standard items' do
      ...
      it 'item should lose 1 quality if normal item' do
        items = [Item.new("foo", 3, 3)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 2
      end

      it 'item should lose 2 quality if sell by date is gone on normal item' do
        items = [Item.new("foo", 0, 3)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 1
      end

      it 'item should not lose quality if quality is already 0' do
        items = [Item.new("foo", 0, 0)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 0
      end

      it 'item should have a decreasing sell_in value' do
    ...
    context 'Backstage passes' do
      ...
      it 'should limit quality at 50 of backstage passes' do
        items = [Item.new("Aged Brie", 2, 50)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 50
      end

      it 'should increase quality by 1 if sell in is 10 or less' do
        items = [Item.new("Backstage passes to a TAFKAL80ETC concert", 15, 4)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 5
      end

      it 'should increase quality by 2 if sell in is 10 or less' do
        items = [Item.new("Backstage passes to a TAFKAL80ETC concert", 8, 4)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 6
      end

      it 'should increase quality by 3 if sell in is 5 or less' do
        items = [Item.new("Backstage passes to a TAFKAL80ETC concert", 4, 4)]
        GildedRose.new(items).update_quality()
        expect(items[0].quality).to eq 7
      end

      it 'should drop quality to 0 once sell in is 0' do
